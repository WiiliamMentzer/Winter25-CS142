...
public class GroceryListProgram {
        ...
        while (addedLastItem) {
            ...
            switch (userChoice) {
                case 1:
                    ...
                    System.out.println("We couldn't add that last item because your list is out of space. The program will now exit");
    ...
    public static void testClasses() {
        ...
        int numberOfFailedTests = 0;
            ...
            GroceryItemOrder bananas = new GroceryItemOrder("Banana",
                    BANANAS_QUANTITY_STARTING, BANANA_COST);

            if (bananas.getCost() == BANANAS_QUANTITY_STARTING * BANANA_COST)
                System.out.println(
                        "Test Passed: bananas.getCost correctly returned " +
                                BANANAS_QUANTITY_STARTING * BANANA_COST);
            else {
                System.out.println("TEST FAILED: bananas.getCost should have returned "
                        + BANANAS_QUANTITY_STARTING * BANANA_COST + " but instead actually returned "
                        + bananas.getCost());
                numberOfFailedTests++;
            }
            ...
            System.out.println("\nAttempted to change the quantity of bananas to " +
            ...
            if (bananas.getCost() == BANANAS_QUANTITY_ENDING * BANANA_COST)
                System.out.println(
                        "Test Passed: bananas.getCost correctly returned " + BANANAS_QUANTITY_ENDING
                                * BANANA_COST);
            else {
                System.out.println("TEST FAILED: bananas.getCost should have returned "
                        + BANANAS_QUANTITY_ENDING * BANANA_COST + " but instead actually returned " +
                        bananas.getCost());
                numberOfFailedTests++;
            }
            ...
            if (singleItemList.getTotalCost() == bananas.getCost())
                ...
                System.out.println("TEST FAILED: singleItemList.getTotalCost() should have returned " + bananas.getCost()
                        + ", but instead actually returned " + singleItemList.getTotalCost());
                numberOfFailedTests++;
            }

            GroceryItemOrder apple = new GroceryItemOrder("Apple", 1, 1.70);
            GroceryItemOrder orange = new GroceryItemOrder("Orange", 3, 1.50);
            double threeItemListTotalCost = apple.getCost() + bananas.getCost() +
                    orange.getCost();
            ...
            if (threeItemList.getTotalCost() == threeItemListTotalCost)
                System.out.println(
                        "Test Passed: threeItemList.getTotalCost() correctly returned " +
                                threeItemList.getTotalCost());
            else {
                System.out.println("TEST FAILED: threeItemList.getTotalCost() should have returned "
                        + threeItemListTotalCost + ", but instead actually returned " +
                        emptyList.getTotalCost());
                numberOfFailedTests++;
            }
            ...
            double fullListTotalCost = apple.getCost() + bananas.getCost() +
                    orange.getCost() + pear.getCost()
                    + baklava.getCost();
            ...
            if (fullList.getTotalCost() == fullListTotalCost)
                System.out.println("Test Passed: fullList.getTotalCost() correctly returned "
                        + fullList.getTotalCost());
            else {
